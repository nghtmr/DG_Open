...
# Gunicorn run
/opt/venv/bin/gunicorn --workers=4 -k uvicorn.workers.UvicornH11Worker --bind 0.0.0.0:8000 -m 007 --log-level debug --access-logfile /app/uvicorn_logs.txt --error-logfile /app/uvicorn_logs.txt --log-file /app/uvicorn_logs.txt --capture-output django_core.wsgi:application &
